...
```{r}
scmix_corralm <- corralm(lapply(scmix_dat, counts))
scmix_fplots <- list()

gg_corralm1 <- plot_embedding(scmix_corralm$v, ellipse_vec = scmix_labs, 
               color_vec = rep(rownames(scmix_corralm$batch_sizes),
                               scmix_corralm$batch_sizes[,2]),
               color_title = 'batch',
               dimname = 'corralm',
               plot_title = 'corralm: counts', returngg = T)

plot_embedding(scmix_corralm$v, color_vec = scmix_labs, 
               ellipse_vec = rep(rownames(scmix_corralm$batch_sizes),
                               scmix_corralm$batch_sizes[,2]),
               color_title = 'Cell line',
               dimname = 'corralm',
               plot_title = 'corralm: counts')

plot(scmix_corralm$d)

gg_corralm2 <- plot_embedding(scmix_corralm$v, ellipse_vec = scmix_labs, 
               color_vec = rep(rownames(scmix_corralm$batch_sizes),
                               scmix_corralm$batch_sizes[,2]),
               color_title = 'batch',
               dimname = 'corralm',
...
dataset_vec <- rep(rownames(scmix_corralm$batch_sizes),
                               scmix_corralm$batch_sizes[,2])
...
library(harmony)
scmix_corralmHM <- HarmonyMatrix(scmix_corralm$v, dataset_vec, do_pca = FALSE)

gg_corralmHM1<- plot_embedding(scmix_corralmHM, color_vec = dataset_vec, ellipse_vec = scmix_labs,
               plot_title = 'corralm + HarmonyMatrix',
               dimname = 'corralm_harmony',
               color_title = 'batch', xpc = 1, returngg = T)
gg_corralmHM2<- plot_embedding(scmix_corralmHM, color_vec = dataset_vec, ellipse_vec = scmix_labs,
               plot_title = '',
               dimname = 'corralm_harmony',
               color_title = 'batch', xpc = 2, returngg = T)
...
```{r}
scmix_HM <- HarmonyMatrix(Reduce(cbind, lapply(scmix_dat, logcounts)), dataset_vec)
gg_HM1 <- plot_embedding(scmix_HM, color_vec = dataset_vec, ellipse_vec = scmix_labs,
               plot_title = 'HarmonyMatrix',
               dimname = 'HarmonyMatrix',
               color_title = 'batch', xpc = 1, returngg = T)
gg_HM2 <- plot_embedding(scmix_HM, color_vec = dataset_vec, ellipse_vec = scmix_labs,
               plot_title = '',
               dimname = 'HarmonyMatrix',
               color_title = 'batch', xpc = 2, returngg = T)
